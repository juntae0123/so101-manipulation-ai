...
v10 실데이터 20편 · 비교 5648건에서 앵커 오차 중앙·최대 모두 **0.0000 mm**,
누적 59.7 mm, 기존 배포 공식 44.8 mm. k에 따라 0 → 40 → 98 → 171 mm 로 증가.

## 되돌릴 조건

- 나머지 54편 또는 다른 수집 배치에서 앵커 오차가 0 이 아니게 나오면
...
- 남은 것: 매니페스트에 앵커 규약을 명시하는 필드가 없다. `compose` 문자열만으로는 구분 불가
