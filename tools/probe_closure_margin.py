...
⚠️ 정정 (2026-09-13): 초판 docstring 은 "f=0.35 의 환산값이 마커 실측과 만나고
두 경로가 독립"이라고 적었다. 둘 다 틀렸다. 실데이터의 그리퍼 채널은 관절각이 아니라
`gap_m` 0~90mm 정규화이고(황도경 회신), 옳게 환산한 최소 명령 개구는 **31.9mm**
(시뮬 f 로는 약 0.49)다. 또 그 채널은 `gripper.csv` 에서 그대로 나온 값이라
마커 실측과 **독립이 아니다**. f=0.80 에서 이미 0% 였으므로 동기는 유지된다.

⚠️ 시뮬과 계약이 그리퍼 정규화를 다르게 쓴다 — 같은 값 -1 이 시뮬에서는 개구 5.3mm,
계약에서는 0mm 다. 이 도구는 **시뮬 안에서 mm 로** 재므로 그 자체로 유효하지만,
결과를 실물 수집 규격으로 옮기려면 두 규약의 대응이 먼저 실측돼야 한다.
