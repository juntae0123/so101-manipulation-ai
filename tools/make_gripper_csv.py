...
🛑 **폐기 (2026-09-12). 학습 데이터 생성에 쓰지 마라.**
   황도경이 71편 전체 `gripper.csv` 를 냈다 (SHA 12cdbc68…, `umi` a6696bdb).
   **그쪽이 정본이다.** 이 파일은 교차검정 fixture 로만 남긴다.

   71편 전수 대조 결과 🟢 `MEASURE_gripper_crosscheck_0912.md`:
     둘 다 검출     5,055 / 6,087 프레임 (83.0%)
     도경만 검출      704   ← 내 필터가 과하게 버린다
     내것만 검출       94
     gap 차이       중앙 1.26mm · p99 10.14mm · 최대 23.72mm
     편별 비율      중앙 1.023 · 범위 0.981 ~ 1.187

   왜 폐기하는가. 단일 에피소드 fixture 는 중앙오차 0.348mm 로 통과했는데
   71편에서는 중앙 1.26mm 다. `SCALE_CORRECTION` 을 그 **한 편에** 적합했기
   때문이다 — 적합에 쓴 편(`rec_20260911_150920`)의 비율은 1.005 이고
   나머지 70편 중앙은 1.023 이다. **n=1 에 맞춘 상수를 검정한 fixture 가
   같은 n=1 이면 그 검정은 아무것도 검정하지 않는다.**
   게다가 편별 비율이 0.981~1.187 로 벌어져 어떤 전역 상수로도 못 맞춘다
   (전역 재척도 후 잔차 중앙 1.13mm). 원인은 척도가 아니라 **프레임 선택**이다.
